<template>
    <v-col class="maincol" :md="archivesMode || (alertEmpty && !$isAdmin) ? 12 : 8" sm="12">
        <v-card outlined elevation="0" style="height:100%" data-intro="Dans cette section sont disponibles les 5 dernières actualités de la CA2BM, 
                ...
                <v-hover v-slot="{ hover }">
                    <v-carousel :cycle="!hover" :continuous="true" :show-arrows="false" hide-delimiters
                        ...
                        <v-carousel-item v-for="(n, x) in news" :key="x">
                            <v-card class="mx-auto" elevation="4" :color="$vuetify.theme.dark ? '#0eb4eda1' : ''"
                ...
                <v-list color="transparent">
                    ...
                    <v-list-item v-for="(archive, i) in archives" :key="i">
                        <v-lazy :options="{
                            ...
                            <div>
                                <v-card class="mb-2" v-for="(a, y) in archive" :key="y"
                                    :color="$vuetify.theme.dark ? '#0eb4eda1' : ''" @click="focus = i + '-' + y">
                                    <v-carousel :continuous="false" :show-arrows="true" hide-delimiter-background
                                        delimiter-icon="mdi-minus" height="400"
                                        v-if="a.photo.length > 1 && focus == i + '-' + y">
                                        <v-carousel-item v-for="(photo, i) in a.photo" :key="i"
                                            @click="LightBoxDialog = true; LightBoxPhotos = a.photo">
                                            <v-img height="400" :src="photo"></v-img>
                                        </v-carousel-item>
                                    </v-carousel>
                                    <v-img height="400" :src="a.photo[0]"
                                        v-if="a.photo.length == 1 && focus == i + '-' + y"
                                        @click="LightBoxDialog = true; LightBoxPhotos = a.photo"></v-img>
                                    <v-card-title class="text-truncate">{{ a.title }}
                                        ...
                                        <v-btn small rounded icon class="mr-1" v-if="a.visible == '0' && $isAdmin"
                                            @click="openDialog = 2, newsToUpdate = a">
                                            <v-icon>mdi-eye-off</v-icon>
                                        </v-btn>
                                        <admin-menu menuType="news" @open="openDialog = $event; newsToUpdate = a"
                                            :pin="a.pinned == '1'" :published="a.visible == '1'" v-if="$isAdmin">
                                        </admin-menu>
                                    </v-card-title>
                                    <v-card-subtitle>{{ a.subtitle }}</v-card-subtitle>
                                    <v-card-text
                                        :class="focus == i + '-' + y ? 'news-text' : 'archivetext text-truncate'"
                                        v-html="a.text" style="overflow: hidden;max-width: 100%;"></v-card-text>
                                </v-card>
                            </div>
                        </v-lazy>
                    </v-list-item>
        ...
        <vue-easy-lightbox escDisabled moveDisabled :visible="LightBoxDialog" :imgs="LightBoxPhotos" :index="0"
            @hide="LightBoxDialog = false"></vue-easy-lightbox>
        <admin-change :open="openDialog == 3" @close="openDialog = -1" @changed="prepare_pin"
            :title="'Modification de l\'épinglage de l\'actualité'"
            ...
            :validate="newsToUpdate.pinned == '1' ? 'Désépingler' : 'Epingler'"
            :color="newsToUpdate.pinned == '1' ? 'red darken-1' : 'green darken-1'" v-if="$isAdmin">
        </admin-change>
        <admin-change :open="openDialog == 2" @close="openDialog = -1" @changed="prepare_visible"
            :title="'Modification de la visibilité de l\'actualité'"
...
export default {
    components: { adminMenu, newsModify, AdminChange },
    name: "News",
    props: {
        alertEmpty: Boolean
    },
